...
from .ha_client import MAX_INTERVAL_MS, execute_scene_actions
...
class SceneRuntimeManager:
    ...
    async def stop(self, scene_id: str) -> dict[str, Any]:
        task = self._tasks.get(scene_id)
    ...
    async def stop_all(self) -> None:
        await asyncio.gather(*(self.stop(scene_id) for scene_id in list(self._tasks)), return_exceptions=True)
